...
/**
 * The ServiceInfoResolver queries up to three times consecutively for a service
 * info, and then removes itself from the timer.
 * <p/>
 * The ServiceInfoResolver will run only if JmDNS is in state ANNOUNCED. REMIND:
 * Prevent having multiple service resolvers for the same info in the timer
 * queue.
 */
public class ServiceInfoResolver extends DNSResolverTask {
	...
	public ServiceInfoResolver(JmDNSImpl jmDNSImpl, ServiceInfoImpl info) {
		...
		this.getDns().addListener(
				info,
				DNSQuestion.newQuestion(info.getQualifiedName(),
						DNSRecordType.TYPE_ANY, DNSRecordClass.CLASS_IN,
		...
		if (!_info.hasData()) {
			...
			newOut = this.addAnswer(
					...
					(DNSRecord) this
							.getDns()
							.getCache()
							.getDNSEntry(_info.getQualifiedName(),
			...
			if (_info.getServer().length() > 0) {
				for (DNSEntry addressEntry : this
						.getDns()
						.getCache()
						.getDNSEntryList(_info.getServer(),
								DNSRecordType.TYPE_A, DNSRecordClass.CLASS_IN)) {
					newOut = this.addAnswer(newOut, (DNSRecord) addressEntry,
							now);
				}
				for (DNSEntry addressEntry : this
						.getDns()
	...
	protected DNSOutgoing addQuestions(DNSOutgoing out) throws IOException {
		...
		if (!_info.hasData()) {
			newOut = this.addQuestion(newOut, DNSQuestion.newQuestion(
					_info.getQualifiedName(), DNSRecordType.TYPE_SRV,
					DNSRecordClass.CLASS_IN, DNSRecordClass.NOT_UNIQUE));
			newOut = this.addQuestion(newOut, DNSQuestion.newQuestion(
					_info.getQualifiedName(), DNSRecordType.TYPE_TXT,
					DNSRecordClass.CLASS_IN, DNSRecordClass.NOT_UNIQUE));
			if (_info.getServer().length() > 0) {
				newOut = this.addQuestion(newOut, DNSQuestion.newQuestion(
						_info.getServer(), DNSRecordType.TYPE_A,
						DNSRecordClass.CLASS_IN, DNSRecordClass.NOT_UNIQUE));
				newOut = this.addQuestion(newOut, DNSQuestion.newQuestion(
						_info.getServer(), DNSRecordType.TYPE_AAAA,
						DNSRecordClass.CLASS_IN, DNSRecordClass.NOT_UNIQUE));
			}
		}
		return newOut;
	...
	protected String description() {
		return "querying service info: "
				+ (_info != null ? _info.getQualifiedName() : "null");
	}
