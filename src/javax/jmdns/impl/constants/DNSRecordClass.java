...
public enum DNSRecordClass {
	...
	private static Logger logger = Logger.getLogger(DNSRecordClass.class
			.getName());
	...
	/**
	 * For answers the top bit indicates that all other cached records are now
	 * invalid.<br/>
	 ...
	 * 
	 * @param index
	 * @return <code>true</code> is the class is unique, <code>false</code>
	 *         otherwise.
	 */
	...
	public static DNSRecordClass classForName(String name) {
		...
		logger.log(Level.WARNING, "Could not find record class for name: "
				+ name);
	...
	public static DNSRecordClass classForIndex(int index) {
		...
		logger.log(Level.WARNING, "Could not find record class for index: "
				+ index);
		return CLASS_UNKNOWN;
	}
