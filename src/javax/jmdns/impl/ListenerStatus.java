 ...
 * The main purpose of this class is to collapse consecutive events so that we
 * can guarantee the correct call back sequence.
 * 
 * @param <T>
 *            listener type
 */
public class ListenerStatus<T extends EventListener> {
	...
	public static class ServiceListenerStatus extends
			ListenerStatus<ServiceListener> {
		private static Logger logger = Logger
				.getLogger(ServiceListenerStatus.class.getName());
		 ...
		 * <b>Note:</b>This event is only the service added event. The service
		 * info associated with this event does not include resolution
		 * information.<br/>
		 ...
		 * 
		 * @param event
		 *            The ServiceEvent providing the name and fully qualified
		 *            type of the service.
		 */
		void serviceAdded(ServiceEvent event) {
			String qualifiedName = event.getName() + "." + event.getType();
			if (null == _addedServices.putIfAbsent(qualifiedName, event
					.getInfo().clone())) {
				...
				logger.finer("Service Added called for a service already added: "
						+ event);
			}
		}
		...
		void serviceRemoved(ServiceEvent event) {
			String qualifiedName = event.getName() + "." + event.getType();
			if (_addedServices.remove(qualifiedName,
					_addedServices.get(qualifiedName))) {
				this.getListener().serviceRemoved(event);
			} else {
				logger.finer("Service Removed called for a service already removed: "
						+ event);
			}
		}
		 ...
		 * <b>Note:</b>This call back will never be called if the service does
		 * not resolve.<br/>
		 * 
		 * @param event
		...
		synchronized void serviceResolved(ServiceEvent event) {
			...
			if ((info != null) && (info.hasData())) {
				String qualifiedName = event.getName() + "." + event.getType();
				ServiceInfo previousServiceInfo = _addedServices
						.get(qualifiedName);
				if (!_sameInfo(info, previousServiceInfo)) {
					if (null == previousServiceInfo) {
						if (null == _addedServices.putIfAbsent(qualifiedName,
								info.clone())) {
							this.getListener().serviceResolved(event);
						}
					} else {
						if (_addedServices.replace(qualifiedName,
								previousServiceInfo, info.clone())) {
							this.getListener().serviceResolved(event);
						}
					}
				} else {
					logger.finer("Service Resolved called for a service already resolved: "
							+ event);
				}
			} else {
				logger.warning("Service Resolved called for an unresolved event: "
						+ event);

			}
		}

		private static final boolean _sameInfo(ServiceInfo info,
	...
	public static class ServiceTypeListenerStatus extends
			ListenerStatus<ServiceTypeListener> {
		private static Logger logger = Logger
				.getLogger(ServiceTypeListenerStatus.class.getName());
		...
		void serviceTypeAdded(ServiceEvent event) {
			if (null == _addedTypes.putIfAbsent(event.getType(),
					event.getType())) {
				this.getListener().serviceTypeAdded(event);
			} else {
				logger.finest("Service Type Added called for a service type already added: "
						+ event);
			}
		}
		...
		void subTypeForServiceTypeAdded(ServiceEvent event) {
			if (null == _addedTypes.putIfAbsent(event.getType(),
				...
				logger.finest("Service Sub Type Added called for a service sub type already added: "
						+ event);
			}
		}
	...
	public boolean equals(Object obj) {
		return (obj instanceof ListenerStatus)
				&& this.getListener().equals(
						((ListenerStatus<?>) obj).getListener());
	}
