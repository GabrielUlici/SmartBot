...
public abstract class JmDNS implements Closeable {
	...
	public static interface Delegate {
		 ...
		 */
		public void cannotRecoverFromIOError(JmDNS dns,
	 ...
	 * {@link #create(InetAddress, String)}.<br/>
	 * Check that your platform correctly handle the default localhost IP
	 ...
	 * given its IP-address.
	 * </p>
	 * If <code>addr</code> parameter is null this method will try to resolve to
	 * a local IP address of the machine using a network discovery:
	 * <ol>
	 * <li>Check the system property <code>net.mdns.interface</code></li>
	 * <li>Check the JVM local host</li>
	 * <li>Use the {@link NetworkTopologyDiscovery} to find a valid network
	 * interface and IP.</li>
	 * <li>In the last resort bind to the loopback address. This is non
	 * functional in most cases.</li>
	 * </ol>
	 * If <code>name</code> parameter is null will use the hostname. The
	 * hostname is determined by the following algorithm:
	 * <ol>
	 * <li>Get the hostname from the InetAdress obtained before.</li>
	 * <li>If the hostname is a reverse lookup default to
	 * <code>JmDNS name</code> or <code>computer</code> if null.</li>
	 * <li>If the name contains <code>'.'</code> replace them by
	 * <code>'-'</code></li>
	 * <li>Add <code>.local.</code> at the end of the name.</li>
	 * </ol>
	 * <p>
	 * <b>Note:</b> If you need to use a custom {@link NetworkTopologyDiscovery}
	 * it must be setup before any call to this method. This is done by setting
	 * up a {@link NetworkTopologyDiscovery.Factory.ClassDelegate} and
	 * installing it using
	 * {@link NetworkTopologyDiscovery.Factory#setClassDelegate(NetworkTopologyDiscovery.Factory.ClassDelegate)}
	 ...
	 * <p>
	 * <b>Note:</b> Unregistered services will not disappear form the list of
	 * services immediately. According to the specification, when unregistering
	 * services we send goodbye packets and then wait <b>1s</b> before purging
	 * the cache.<br/>
	 ...
	 * service listeners will be notified of the new service type.
	 * <p>
	 ...
	 *            full qualified service type, such as
	 *            <code>_http._tcp.local.</code>.
	 * @return <code>true</code> if the type or subtype was added,
	 *         <code>false</code> if the type was already registered.
	 */
